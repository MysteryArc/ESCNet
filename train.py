...
def main():
    ...
    train_set = GetDataset("C:\\Users\\why\\Desktop\\xView2dataset\\train_cropped")
    val_set = GetDataset("C:\\Users\\why\\Desktop\\xView2dataset\\val_cropped")
    ...
    for i in range(NUM_EPOCH):
        ...
        for pre_image, post_image, _, damage_target in tqdm(train_loader, desc='Epoch {} Training'.format(i + 1)):
            # 加载图像
        ...
        val_loss = 0
        conf_matrix = np.zeros((NUM_CLASSES, NUM_CLASSES), dtype=np.int64)
        for pre_image, post_image, building_target, damage_target in tqdm(val_loader, desc='Epoch {} Validating'.format(i + 1)):
            # 加载图像
            pre_image = pre_image.to(DEVICE)
            post_image = post_image.to(DEVICE)
            target = damage_target.to(DEVICE).long()
